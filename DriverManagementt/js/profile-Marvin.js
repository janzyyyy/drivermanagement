...
(function() {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        loadProfile(); // Load profile data on page start
        openProfileSocket(); // Initialize WebSocket for profile updates
    });
    ...
    function initEditAccountModal() {
        ...
        // Wire edit modal form submit to saveProfile API
    ...
    function initEditDriverModal() {
        ...
        // Wire edit driver modal form submit to saveProfile API
        editForm.addEventListener('submit', async (e) => {
            e.preventDefault();
            const payload = {
                ...
                yearsExperience: document.getElementById('editYearsExperience').value,
                previousJobs: document.getElementById('editPreviousJobs').value,
                vehicleCategory: Array.from(document.querySelectorAll('#vehicleDropdownMenu input[type="checkbox"]'))
            ...
            try {
                ...
                showToast('Failed to save driver profile');
            }
        });
    }
    ...
    async function loadProfile() {
        try {
            const res = await fetch('../php/api/profile.php', { credentials: 'include' });
            if (!res.ok) throw new Error('Failed to load profile');
            const data = await res.json();
            // map returned fields into your DOM (example)
    ...
    async function saveProfile(payload) {
        const res = await fetch('../php/api/profile.php', {
            method: 'PUT',
    ...
    // Wire edit account modal submit
        ...
        if (editAccountForm) {
            editAccountForm.addEventListener('submit', async (e) => {
                ...
                try {
                    await saveProfile(payload);
                    await loadProfile();
                    // close modal (implement your modal closing code)
        ...
        if (editDriverForm) {
            editDriverForm.addEventListener('submit', async (e) => {
                ...
                try {
                    await saveProfile(payload);
                    await loadProfile();
                    document.getElementById('closeDriverModalBtn')?.click();
                } catch (err) {
    ...
    // WebSocket for profile updates
    let profileSocket;

    function openProfileSocket() {
        profileSocket = new WebSocket('wss://example.com/ws/profile'); // adjust URL
        profileSocket.addEventListener('open', () => {
            console.log('Profile socket opened');
            // optionally authenticate
            // profileSocket.send(JSON.stringify({ type: 'auth', token: '...' }));
        });
        profileSocket.addEventListener('message', (ev) => {
        ...
        profileSocket.addEventListener('close', () => console.log('Profile socket closed'));
        profileSocket.addEventListener('error', (err) => console.error('Socket error', err));
    }

    // Open profile WebSocket on DOMContentLoaded
    document.addEventListener('DOMContentLoaded', openProfileSocket);
